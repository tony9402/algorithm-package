...
vector<int> coloring(int N, const vector<pair<int, int>> &edges) {
    ...
    function<void(int,int,int)> dfs = [&](int cur, int prev, int c) -> void {
        if(rnd.next(0, 1)) color[cur] = rnd.next(c, N);
        else color[cur] = color[prev];
        for(int nxt : tree[cur]) {
            if(nxt == prev) continue;
            dfs(nxt, cur, !!color[cur]);
        }
    };
    dfs(1, 0, 0);
...
void random_skewed_generate(int N) {
    ...
    if(N > 2) {
        int cnt = rnd.next(15, 100);
        for(int i = 0; i < cnt; i++) {
            int a = rnd.next(2, N);
            int b = rnd.next(2, N);
            while(a == b) b = rnd.next(2, N);
            swap(nodes[a - 1], nodes[b - 1]);
        }
    }
